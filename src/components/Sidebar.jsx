...
const NavSkeleton = ({ count = 1 }) => (
    <>
        {Array.from({ length: count }).map((_, i) => (
            <div
                key={i}
                className="flex items-center mx-auto my-0.5 w-56 px-6 py-2 rounded-md gap-3"
            >
                {/* Icon placeholder */}
...
const Sidebar = () => {
    const { user, loading } = useUsers(); // make sure useUsers exposes loading
    ...
    return (
        <aside className="w-64 min-h-screen bg-white border border-gray-200">
            ...
            <nav className="mt-2 flex flex-col text-sm">
                {loading ? (
                    // Skeleton: 6 nav items (matches max Admin/Reviewer links) + Ticket
                    <>
                        <NavSkeleton count={6} />
                    ...
                    <>
                        ...
                        {isMentor && (
                            <>
                                ...
                                <NavLink to="admin-apprentice" className={linkClass}>
                                    <RiGraduationCapLine className="text-lg -mt-0.5 mr-1 opacity-90" />
                                    <span className="ml-3">Applications</span>
                                </NavLink>
                            </>
                        )}
